...
read_csv(here("data/adhis_NLS_data.csv")) %>%
  ...
  rowwise() %>%
  mutate(total_incarcerated = sum(c_across(starts_with("E")), na.rm = TRUE)) %>%
  ungroup() %>%
  ...
  select(race, gender, total_incarcerated) %>%
  
  # write to a csv
  write_csv(here("data/adhis_NLS_data_clean.csv"))